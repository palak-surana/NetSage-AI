...
def check_gateway(text, findings):
    ...
    Handles evidence where the expected gateway is in the
    topology note and the configured gateway is in show output.
    """
    ...
    expected_match = re.search(
        r"(?:gateway\s+should\s+be|expected\s+(?:default\s+)?gateway)"
        r"\s*[:=]?\s*"
        r"(\d{1,3}(?:\.\d{1,3}){3})",
        lower
    )
    ...
    if expected_match and configured_match:

        expected = expected_match.group(1)
        configured = configured_match.group(1)
        ...
        if configured != expected:
            ...
            add_finding(
                ...
                (
                    f"Configured gateway: {configured}; "
                    f"Expected gateway: {expected}"
                )
            )
...
def check_speed_duplex(text, findings):
    ...
    # -----------------------------------------------------
    # Detect configured speed vs expected speed
    #
    # Example:
    # configured speed 10
    # expected speed 100
    ...
    speed_match = re.search(
        r"(?:configured|current)\s+speed\s+"
        r"(\d+)"
        r".*?"
        r"(?:expected|correct|actual)\s+speed\s+"
        r"(\d+)",
        lower
    )
    ...
    if speed_match:
        ...
        configured = speed_match.group(1)
        expected = speed_match.group(2)
    ...
    # -----------------------------------------------------
    # Detect Cisco show interface format
    #
    # CASE027:
    # speed 10Mb/s, duplex half
    # expected 100Mb/s full
    # -----------------------------------------------------

    actual_speed = re.search(
        r"speed\s+(\d+)\s*mb/s",
        lower
    )

    actual_duplex = re.search(
        r"duplex\s+(half|full)",
        lower
    )

    expected_speed = re.search(
        r"expected\s+(\d+)\s*mb/s",
        lower
    )

    expected_duplex = re.search(
        r"expected\s+\d+\s*mb/s\s+(half|full)",
        lower
    )
    ...
    if (
        ...
        configured_speed = actual_speed.group(1)
        configured_duplex = actual_duplex.group(1)

        expected_speed_value = expected_speed.group(1)
        expected_duplex_value = expected_duplex.group(1)

        if (
            configured_speed != expected_speed_value
            or configured_duplex != expected_duplex_value
        ):
            ...
            add_finding(
                ...
                (
                    "Speed or duplex configuration mismatch detected: "
                    f"configured {configured_speed}Mb/s "
                    f"{configured_duplex}; "
                    f"expected {expected_speed_value}Mb/s "
                    f"{expected_duplex_value}."
                ),
                (
                    f"Configured: {configured_speed}Mb/s "
                    f"{configured_duplex}; "
...
def check_vlan_assignment(text, findings):
    ...
    #
    # Evidence:
    # Fa0/1 should be an access port in VLAN 10
    # Fa0/1 is listed under VLAN 1
    #
    # We intentionally search the actual and expected VLAN
    # independently because they may appear in different
    # fields of the dataset.
    # -----------------------------------------------------

    # Find actual VLAN from show vlan brief
    actual_match = re.search(
        r"(?:fa\d+/\d+|gi\d+/\d+|g\d+/\d+)"
        r"\s+is\s+listed\s+under\s+vlan\s+(\d+)",
        lower
    )

    # Find expected VLAN from topology/evidence
    expected_match = re.search(
        r"(?:access\s+port\s+in\s+vlan|should\s+be\s+an?\s+access\s+port\s+in\s+vlan)"
        r"\s*(\d+)",
        lower
    )

    # More flexible expected VLAN pattern
    if not expected_match:

        expected_match = re.search(
            r"should\s+be\s+(?:an?\s+)?access\s+port"
            r".*?"
            r"vlan\s+(\d+)",
            lower
        )
    ...
    if actual_match and expected_match:
        ...
        if actual != expected:
            ...
            add_finding(
                ...
                "VLAN Configuration",
                (
                    f"Incorrect switchport VLAN assignment: "
                    f"actual VLAN {actual}; "
                    f"expected VLAN {expected}."
                ),
                (
                    f"Port is listed under VLAN {actual}; "
                    f"should be in VLAN {expected}."
                )
            )
