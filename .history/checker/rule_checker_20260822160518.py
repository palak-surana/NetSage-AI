...
def check_gateway(text, findings):
    ...
    # -----------------------------------------------------
    # Numeric gateway comparison
    # -----------------------------------------------------

    gateway_patterns = [

        # configured default gateway 192.168.1.254
        # expected default gateway 192.168.1.1
        r"(?:configured|current)"
        r"\s+default\s+gateway\s+"
        r"(\d+\.\d+\.\d+\.\d+)"
        r".*?"
        r"(?:expected|correct|actual)"
        r"\s+(?:default\s+gateway|gateway)"
        r"\s+"
        r"(\d+\.\d+\.\d+\.\d+)",
        ...
        # configured gateway 192.168.1.254
    ...
    if (
        ...
        or "default gateway is blank" in lower
    ):
...
def check_speed_duplex(text, findings):
    ...
    # -----------------------------------------------------
    # Detect explicit configured/expected speed
    # -----------------------------------------------------
...
def check_vlan_assignment(text, findings):
    ...
    patterns = [
        ...
        # Fa0/1 is listed under VLAN 1
        # should be an access port in VLAN 10
